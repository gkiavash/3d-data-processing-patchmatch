...
#pragma omp parallel for
  ...
  void PatchMatch::spatial_propagation(int x, int y, int cpv, int iter) {
    ...
    //iter --> if iter is an odd number check for left or down else for right or up
      ...
      int propagate_indexes[2][2];
      if(iter%2 == 0) {
          propagate_indexes[0][0] = -1;
          propagate_indexes[0][1] = 0;
          propagate_indexes[1][0] = 0;
          propagate_indexes[1][1] = -1;
      ...
      for (auto & propagate_ind : propagate_indexes){
          int x_neigh = x + propagate_ind[0];
          int y_neigh = y + propagate_ind[1];

          if(!inside(x_neigh, y_neigh, 0, 0, views_[cpv].cols, views_[cpv].rows))
              continue;

          float new_disp = disps_[cpv](y_neigh, x_neigh);
          float new_cost = precomputed_disp_match_cost(new_disp, x, y, cpv);

          if (new_cost < current_cost) {
              disps_[cpv](y, x) = new_disp;
              costs_[cpv](y, x) = new_cost;
          }
      }
  ...
  void PatchMatch::view_propagation(int x, int y, int cpv) {
    ...
    //lbx=0, lby=0, ubx=cols_, uby=rows_
      ...
      float disp_1 = disps_[cpv].at<float>(y, x);

      int x_2 = x + sign * (int) disp_1;

      if(!inside(x_2, y, 0, 0, views_[1-cpv].cols, views_[1-cpv].rows))
          return;

      float cost_1 = costs_[1-cpv](y, x_2);
      float cost_2 = precomputed_disp_match_cost(disp_1, x_2, y, 1-cpv);


      if(cost_2 < cost_1)
      {
//          cout<<"\nbefore: "<<disps_[1-cpv](y, x_2);
          disps_[1-cpv](y, x_2) = disp_1;
          costs_[1-cpv](y, x) = cost_2;
//          cout<<"after: "<<disps_[1-cpv](y, x_2);
      }
  ...
  void PatchMatch::disp_perturbation(int x, int y, int cpv, float max_delta_z, float end_dz) {
    ...
    //to compute costs use precomputed_disp_match_cost
      ...
      while(max_delta_z >= end_dz) {
          ...
          std::uniform_real_distribution<> rand_new_disp(-max_delta_z, +max_delta_z);
          auto rand_new_disp_delta = (float) rand_new_disp(gen);

          float current_disp = disps_[cpv](y, x);
          float current_cost = costs_[cpv](y, x);

          float new_disp = current_disp + rand_new_disp_delta;
          float new_cost = precomputed_disp_match_cost(new_disp, x, y, cpv);

          if (new_cost < current_cost) {
              disps_[cpv](y, x) = new_disp;
              costs_[cpv](y, x) = new_cost;
//              cout<<"\ndecreased "<<current_disp<<","<<new_disp<<" | "<<current_cost<<","<<new_cost;
          }
          max_delta_z /= 2.0f;
      }
